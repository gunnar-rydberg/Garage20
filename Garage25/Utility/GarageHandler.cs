...
namespace Garage20.Utility
{
    public class GarageHandler
    {
        ...
        // Use for building SelectList:s needed for HTML5 select boxes
        ...
        public SelectList GetParkingLots(int vehicleTypeId, string emptyValueName = null)
        {
            ...
            if (size < 1m)
            {
                foreach (var lot in findFreeParkingSubSpace(size))
            ...
            if (size > 1m)
            {
                foreach (var lots in findFreeParkingSpaceMultiple((int)Decimal.Ceiling(size)))
                {
        ...
        /// <summary>
        /// Find a range of free parking lots
        /// </summary>
        private IEnumerable<List<ParkingLot>> findFreeParkingSpaceMultiple(int size)
        {
            //TODO? Optimization? query DB for a List<{Id, bool}> based on free parking space
            //                    then search for adjacent free space in that list
